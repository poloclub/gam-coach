    ...
    for (let j = 0; j < encodedSample.length; j++) {
      ...
      if (curFeatureType === 'continuous') {
        ...
        if (binIndex < 0) {
          ...
          console.log(`Unseen categorical level: ${curFeatureName}, ${j}, ${curFeature}`);
          binScore = 0;
        } else {
          binScore = this.scores[j][binIndex];
        }
...
class EBMLocal extends EBM {
  ...
  constructor(model, sample) {
    ...
    this.predScore = Object.values(this.countedScores).reduce((a, b) => a + b) + this.intercept;
    this.predProb = this.isClassifier ? sigmoid(this.predScore) : this.predScore;
    this.pred = this.isClassifier ? (this.predProb >= 0.5 ? 1 : 0) : this.predScore;
  }
  ...
  updateFeature(name, value) {
    ...
    if (curType !== 'continuous' && curType !== 'categorical') {
      throw new Error('Only continuous and categorical features can be updated');
    }
    ...
    if (curType === 'continuous') {
      ...
      if (binIndex < 0) {
        // Unseen level during training => use 0 as score instead
        console.log(`Unseen categorical level: ${name}, ${index}, ${curFeature}`);
        binScore = 0;
      } else {
        binScore = this.scores[index][binIndex];
      }
...
class MILP {
   ...
   */
  constructor(cfDirection, neededScoreGain, featuresToVary, options,
    maxNumFeaturesToVary = null, mutedVariables = [], verbose = 0) {
  ...
  createMILP() {
    ...
    Object.keys(this.options).forEach(optName => {
      if (optName.includes(' x ')) {
        ...
        if (this.featuresToVary.includes(f1Name) && this.featuresToVary.includes(f2Name)) {
